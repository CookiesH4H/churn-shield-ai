...
print("================================================================================")
print("INICIANDO PIPELINE DE MACHINE LEARNING DESDE MONGODB ATLAS")
print("================================================================================")
...
print(f"DataFrames listos. Ventas entrenamiento: {len(train_sales)}. Ventas predicción: {len(test_sales)}.")
...
def prepare_cut(M, is_prediction=False):
    ...
    if not is_prediction:
        churners = set(train_sales[(train_sales['month_idx'].between(M+1, M+3)) & (train_sales['target'] == 1)]['customer_id'])
...
print(f"ROC-AUC de Validación: {roc_auc_score(y_val, y_pred_prob):.4f}")

# -------------------------------------------------------------
# Paso 5: Predicción y Reglas del Escenario C
# -------------------------------------------------------------
print("\nPaso 4: Calculando proyecciones para Febrero 2026...")
...
# -------------------------------------------------------------
# Paso 6: Actualizar Resultados en MongoDB Atlas y Guardar CSV Local
# -------------------------------------------------------------
print("\nPaso 5: Actualizando la colección 'customers' en MongoDB Atlas...")
bulk_operations = []
for i, row in df_final.iterrows():
    bulk_operations.append(
        UpdateOne(
            ...
            {'$set': {
                'churnRiskScore': float(row['probabilidad_abandono'] * 100),
                'riskLevel': row['nivel_riesgo'],
                'accionComercial': row['accion_comercial'],
                'primaryRiskFactor': row['primary_risk_factor'],
                'churnReason': row['razon_abandono']
            }}
        )
    )
...
if bulk_operations:
    ...
    print(f"  Modificados en MongoDB: {result.modified_count} documentos en {time.time() - t_mongo_start:.2f} segundos.")
...
output_path = os.path.join(folder, "scoring_clientes_feb2026_sample.csv")
try:
    final_csv.to_csv(output_path, index=False)
    print(f"Respaldo local guardado en: {output_path}")
except PermissionError:
    alt_path = os.path.join(folder, "scoring_clientes_feb2026_sample_backup.csv")
    try:
        final_csv.to_csv(alt_path, index=False)
        print(f"Advertencia: El archivo principal estaba bloqueado. Respaldo alternativo guardado en: {alt_path}")
    except Exception as e:
